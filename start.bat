...
function Load-Tokens {
    ...
    return [pscustomobject]@{ total_tokens = 0; session_tokens = 0 }
}
...
function Run-Installer {
    ...
    if (-not (Test-Path $TokensPath)) {
        try {
            [ordered]@{ total_tokens = 0; session_tokens = 0 } | ConvertTo-Json |
...
function Run-Launcher {
    ...
    function Draw-Dashboard($cfg, $status) {
        ...
        $tok = Load-Tokens
        $totalTokensFmt   = "{0:N0}" -f [long]$tok.total_tokens
        $sessionTokensFmt = "{0:N0}" -f [long]$tok.session_tokens
        $totalCost        = [double]$tok.total_tokens * 0.00003
        $sessionCost      = [double]$tok.session_tokens * 0.00003
        $totalCostFmt     = "{0:N4}" -f $totalCost
        $sessionCostFmt   = "{0:N4}" -f $sessionCost

        Write-Host "  [>] In your entire career, you've saved:          " -NoNewline -ForegroundColor DarkGray
        Write-Host "$totalTokensFmt tokens " -NoNewline -ForegroundColor Magenta
        Write-Host "which is worth " -NoNewline -ForegroundColor DarkGray
        Write-Host "`$$totalCostFmt " -NoNewline -ForegroundColor Green
        Write-Host "(GPT-4 classic scale)" -ForegroundColor DarkGray

        Write-Host "  [>] In this session only:                         " -NoNewline -ForegroundColor DarkGray
        Write-Host "$sessionTokensFmt tokens " -NoNewline -ForegroundColor DarkGray
        Write-Host "which is worth " -NoNewline -ForegroundColor DarkGray
        Write-Host "`$$sessionCostFmt" -ForegroundColor DarkGray
        Write-Host ""
        Write-Host "  -----------------------------------------------------------------" -ForegroundColor DarkGray
        Write-Host ""
