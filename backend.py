...
async def ollama_bridge(request: Request):
    try:
        ...
        try:
            ...
            # Extract internal thoughts if present and write to thoughts.txt
            ...
            if thoughts:
                try:
                    ...
                    with open(thoughts_path, "w", encoding="utf-8") as tf:
                        tf.write(str(thoughts))
                    print(f"[BRIDGE]: Extracted internal thoughts -> {thoughts_path}")
                except Exception as tf_err:
                    print(f"[BRIDGE WARNING]: Failed to write thoughts.txt: {tf_err}")
